...
func (b *Bucket) Multi(key, contType string, perm ACL) (*Multi, error) {
	...
	return b.InitMulti(key, contType, perm,header)
}
...
func (b *Bucket) InitMulti(key string, contType string, perm ACL, metadata map[string][]string) (*Multi, error) {
	...
	for k,v := range metadata{
		...
		if k[:11] == "x-amz-meta-"{
			...
			headers[k] = append(headers[k],headcontent)
		}
	}
	params := map[string][]string{
		"uploads": {""},
