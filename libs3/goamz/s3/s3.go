...
func (req *request) url(full bool) (*url.URL, error) {
	...
	//logging.Debug("req.path=%s", req.path)
	u.Opaque = amazonEscape(req.path)
	//logging.Debug("u.Opaque=%s", u.Opaque)
	if full {
...
func (s3 *S3) prepare(req *request) error {
	...
	//logging.Debug("req.baseurl=%s, req.signpath=%s", req.baseurl, req.signpath)
	return nil
}
...
func (s3 *S3) run(req *request, resp interface{}) (*http.Response, error) {
	...
	//}
	//if !connection {
	//	req.headers["connection"] = []string{"Keep-Alive"}
	//}
